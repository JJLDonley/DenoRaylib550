...
static char *EscapeBackslashes(char *text);                 // Replace '\' by "\\"
static char *EscapeJsonString(char *text);                  // Escape string for JSON export
...
static char *EscapeJsonString(char *text)
{
    static char buffer[512] = { 0 };

    int count = 0;

    for (int i = 0; (text[i] != '\0') && (count < 511); i++, count++)
    {
        if ((text[i] == '\\') || (text[i] == '"'))
        {
            if (count >= 510) break;
    ...
    switch (format)
    {
        ...
        case JSON:
        {
            ...
            for (int i = 0; i < structCount; i++)
            {
                fprintf(outFile, "    {\n");
                fprintf(outFile, "      \"name\": \"%s\",\n", structs[i].name);
                fprintf(outFile, "      \"description\": \"%s\",\n", EscapeJsonString(structs[i].desc));
                ...
                for (int f = 0; f < structs[i].fieldCount; f++)
                {
                    ...
                    fprintf(outFile, "          \"description\": \"%s\"\n", EscapeJsonString(structs[i].fieldDesc[f]));
                    fprintf(outFile, "        }");
            ...
            for (int i = 0; i < enumCount; i++)
            {
                ...
                for (int e = 0; e < enums[i].valueCount; e++)
                {
                    ...
                    fprintf(outFile, "          \"description\": \"%s\"\n", EscapeJsonString(enums[i].valueDesc[e]));
                    fprintf(outFile, "        }");
            ...
            for (int i = 0; i < callbackCount; i++)
            {
                fprintf(outFile, "    {\n");
                fprintf(outFile, "      \"name\": \"%s\",\n", callbacks[i].name);
                fprintf(outFile, "      \"description\": \"%s\",\n", EscapeJsonString(callbacks[i].desc));
