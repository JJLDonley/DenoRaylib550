...
const player = new RL.Rectangle(400, 280, 40, 40);
const buildings: RL.Rectangle[] = Array.from(
  { length: MAX_BUILDINGS },
  () => new RL.Rectangle(0, 0, 0, 0),
);
const buildColors: RL.Color[] = Array.from(
  { length: MAX_BUILDINGS },
...
for (let i = 0; i < MAX_BUILDINGS; i++) {
  ...
  spacing += Math.trunc(buildings[i].width);

  buildColors[i] = new RL.Color(
